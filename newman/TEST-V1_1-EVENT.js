...
const testFilePrefix = 'TEST-V1_1-EVENT';
...
function step2_sendEvents(args, callback){
    ...
    async.parallel({
        sendADAS: function(innerCallback) {
          sendEvent.sendADAS(mqttArg, innerCallback);
        },
        sendBatteryWarning: function(innerCallback) {
          sendEvent.sendBatteryWarning(mqttArg, innerCallback);
        },
        sendCollisionWarningDriving: function(innerCallback) {
          sendEvent.sendCollisionWarningDriving(mqttArg, innerCallback);
        },
        sendCollisionWarningParking: function(innerCallback) {
          sendEvent.sendCollisionWarningParking(mqttArg, innerCallback);
        },
        sendDiagnostic: function(innerCallback) {
          sendEvent.sendDiagnostic(mqttArg, innerCallback);
        },
        sendTrunoffWarning: function(innerCallback) {
          sendEvent.sendTrunoffWarning(mqttArg, innerCallback);
        },
        sendUnpluggedWarning: function(innerCallback) {
          sendEvent.sendUnpluggedWarning(mqttArg, innerCallback);
        }
...
function step3_checkEvents(args, callback){
    const options = makeNewmanOption(args.environment, 'Check Events');
